...
def main():
    ...
    ap.add_argument("--query", default="xash3d-query",
                    help="path to the xash3d-query binary")
    ap.add_argument("--sources", default="servers",
                    help="directory containing per-gamedir TOML sources")
    ap.add_argument("--output", default="output",
                    help="directory to write the publishable tree into")
    ap.add_argument("--tries", type=int, default=4,
                    help="probe attempts per server")
    ap.add_argument("--timeout", type=int, default=2,
                    help="per-probe response timeout, in whole seconds (passed to xash3d-query -t)")
    ap.add_argument("--grace-hours", type=float, default=48.0,
                    help="keep a silent server published if it responded within this many hours")
    args = ap.parse_args()
    ...
    live_now = 0
    grace_kept = 0
    print(f"probing {total} servers across {len(sources)} gamedirs"
          f"  (tries={args.tries}, timeout={args.timeout}s, grace={args.grace_hours}h)",
          flush=True)
    ...
    for gamedir, entries in sources.items():
        ...
        for entry in entries:
            ...
            if age <= args.grace_hours:
                ...
                print(f"  [-] {gamedir:>12}  {address}  silent (last seen {age:.1f}h ago)" if prev.get("last_seen")
                      else f"  [-] {gamedir:>12}  {address}", flush=True)
    ...
    print(f"done: {live_now} responded, {grace_kept} kept by grace, {pruned} pruned from state",
          flush=True)
    return 0
